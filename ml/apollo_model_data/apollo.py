...
class RMSNorm(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        return input_norm.type_as(input.to(self.weight.device)).reshape(
            B, N, T
...
class RMVN(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        input_norm = input_norm.reshape(B, N, T) * self.std.reshape(1, -1, 1) + self.mean.reshape(
            1, -1, 1
        )
...
class Roformer(nn.Module):
    ...
    def _calc_rotary_emb(self) -> tuple[torch.Tensor, torch.Tensor]:
        freq = 1.0 / (
            self.theta
            ** (torch.arange(0, self.hidden_size, 2)[: (self.hidden_size // 2)] / self.hidden_size)
        )  # theta_i
    ...
    def _add_rotary_emb(self, feature: torch.Tensor, pos: int) -> torch.Tensor:
        ...
        feature_reshape_neg = (
            torch.flip(feature_reshape.reshape(-1, N // 2, 2), [-1]) * reverse_sign.reshape(1, 1, 2)
        ).reshape(-1, N)
    ...
    def __init__(
        ...
        if not causal:
            self.conv = nn.Sequential(
                nn.Conv1d(
                    in_channel, in_channel, kernel, padding=(kernel - 1) // 2, groups=in_channel
                ),
                RMSNorm(in_channel),
...
class Apollo(BaseModel):
    ...
    def spec_band_split(self, input: torch.Tensor) -> tuple[list[torch.Tensor], torch.Tensor]:
        ...
        for i in range(self.nband):
            ...
            subband_power_parts.append(
                (this_spec.abs().pow(2).sum(1) + self.eps).sqrt().unsqueeze(1)
            )  # B, 1, T
            normalized_spec = torch.complex(
                this_spec.real / subband_power_parts[-1], this_spec.imag / subband_power_parts[-1]
            )
