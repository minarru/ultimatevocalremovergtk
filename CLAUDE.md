...
- `core.model_display._merged_for_display()` is memoized (`lru_cache(maxsize=1)`). Anything that replaces catalogue source data must call `clear_display_cache()` **at the point of replacement**, not only from a `clear_*_cache()` entry point callers may never invoke. Adding a catalogue source means adding that call.
