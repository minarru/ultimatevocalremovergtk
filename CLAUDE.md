...
- **`core/`** — Tk-free backend facade. Public surface is re-exported in [core/__init__.py](core/__init__.py): `Settings`, `ModelConfig`/`ModelRepository`/`assemble_model`, `ProcessData`, `JobRunner`/`JobCallbacks` (callbacks live in [core/job_callbacks.py](core/job_callbacks.py); single/ensemble file-pass hooks live in [core/run_hooks.py](core/run_hooks.py)), `AudioToolRunner`.
