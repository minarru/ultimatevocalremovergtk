...
- libadwaita composites hide behaviour in internal children: `Adw.Banner` wraps its content in a `Gtk.Revealer` (animated slide-down), and `Adw.ToolbarView` stacks top bars in a `Gtk.Box` carrying libadwaita's `collapse-spacing` class. Reach them by walking for the type/class and degrade gracefully — they are implementation details.
- Animating an `Adw.ToolbarView` top bar re-allocates all content beneath it every frame (observed during a previous `Adw.Banner` layout investigation). Reveal top bars instantly unless you have measured the reflow.
- `Adw.Dialog` using breakpoints needs an explicit minimum width and height, for example via `set_size_request(...)`. `set_content_width`/`set_content_height` are only *preferred* sizes; without a minimum libadwaita warns ("does not have a minimum size") and can allocate below the content's minimum, clipping children.
