...
* :func:`~uvr_gtk.dialogs.model_params.make_unrecognized_handler` - builds the
  :attr:`uvr_core.ModelRepository.on_unrecognized_model` hook so an unknown
  model (its MD5 missing from the mapper JSONs) prompts for parameters and
  persists them next to the model-data cache.
* :func:`~uvr_gtk.dialogs.model_params.show_change_defaults_dialog` - the
