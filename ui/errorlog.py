...
with *Copy Log* and *Report Issue* actions. This module provides:
...
_ERROR_LOG_VIEW: ErrorLogView | None = None
...
def mock_error_log() -> str:
    """Synthetic, scrollable report for the Inspector-only visual QA action."""
    sections = []
    for number, phase in enumerate(
        ("Loading model", "Reading input", "Processing", "Combining stems", "Saving outputs"),
        start=1,
    ):
        sections.append(
            f"SYNTHETIC TEST ERROR {number} — UI preview only\n"
            "No processing was started and no real operation failed.\n"
            f"Phase: {phase}\n"
            "Model: Example separation model\n"
            "Input: example-track.wav\n"
            "Outputs: Vocals, Instrumental\n"
            "\n"
            "Traceback (synthetic sample):\n"
            '  File "<inspector-preview>", line 42, in example_operation\n'
            '    raise RuntimeError("Example error for layout testing")\n'
            "RuntimeError: Example error for layout testing. This deliberately long "
            "explanation exercises wrapping in the Error Log at narrow window widths.\n"
            "\n"
            "This sample can be cleared with the Clear Log button."
        )
    return "\n\n---\n\n".join(sections)


def open_error_log(parent_window: typing.Any, message: typing.Any = None):
    """Open the non-modal Error Log window. Wire this to a ``win.error_log`` action.
    ...
    global _ERROR_LOG_WINDOW, _ERROR_LOG_BUFFER, _ERROR_LOG_SINK, _ERROR_LOG_VIEW
    ...
    view = ErrorLogView(builder)

    def copy_log(_button: Gtk.Button) -> None:
        copy_text(window, get_error_log())
        view.copied()

    view.connect_copy(copy_log)
    view.clear_action.connect("activate", lambda *_: set_error_log(""))
    ...
    buffer = object_from_builder(builder, "buffer", Gtk.TextBuffer)
    _ERROR_LOG_VIEW = view
    _ERROR_LOG_WINDOW = window
    _ERROR_LOG_BUFFER = buffer
    _ERROR_LOG_SINK = ErrorLogViewSink(buffer, view.update)
...
def _on_error_log_window_closed(_window: typing.Any) -> bool:
    ...
    if _ERROR_LOG_VIEW is not None:
        _ERROR_LOG_VIEW.close()
        _ERROR_LOG_VIEW = None
