...
class EnsembleWeightPreflightTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        routes = (
            ...
            StemRoute(
                ...
                complement_of=vocals,
            ),
        )
        self.members = tuple(
            ModelDescriptor(f"mdx:{name}", "mdx", name, name, routes=routes) for name in ("a", "b")
        )
    ...
    def test_unselected_output_does_not_block(self) -> None:
        ...
        self.assertEqual(
            ensemble_pair_diagnostics(self.settings, self.members, command="ensemble"), ()
        )
