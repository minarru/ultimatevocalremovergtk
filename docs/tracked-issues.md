...
| **F3** | Repeated model-list metadata work | done | medium | n/a — fork-only (feature not in upstream) | — | Closed by the UI metadata cache work: karaoke/BV eligibility is cached per tag set, checkpoint hashes persist as mtime/size-guarded settings entries, and `format_tag_title` is memoized per display generation. |
...
Originally deferred during the 2026-08-04 UI performance pass; closed in the 2026-08-05 follow-up unless noted.
