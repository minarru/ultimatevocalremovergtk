...
| **F5** | Model download can start an infinite GTK model-picker refresh loop | open | **high** | n/a — fork-only (GTK repository notification spine) | — | Confirmed from a user report: during a multi-model download, an open separation model picker repeatedly reset and eventually the whole window stopped responding. `ModelRepository.invalidate_models()` notifies `MainWindow._on_models_changed`; its idle flush calls `_apply_model_refresh`, which calls `invalidate_models()` again ([ui/window.py](../ui/window.py), [core/model_data.py](../core/model_data.py)). The repository's synchronous re-entrancy guard cannot span the idle boundary, so every pass schedules the next. Each pass replaces every `Adw.ComboRow` model via `set_combo_values`, losing popover scroll/search state and starving input/tooltips/window-close handling. Split cache invalidation from consumer repaint: a repository-originated repaint must not invalidate again. Add a test that executes the scheduled callback and proves no second callback is queued. |
| **F6** | Download progress can backlog GTK idle callbacks | open | medium | n/a — fork-only (GTK queue indicator) | — | `DownloadQueue` emits progress notifications at up to 10 Hz plus detail changes, and `init_download_queue_ui` turns every notification into a separate `idle_on_main(indicator.refresh)` ([core/download_queue.py](../core/download_queue.py), [ui/download.py](../ui/download.py)). There is no producer-side pending flag, so a busy main loop accumulates stale renders. Coalesce to one pending idle that reads the latest queue snapshot; terminal/batch callbacks remain lossless. Test a notification burst before draining the fake main loop and assert one render with the newest state. |
| **F7** | Separation progress is throttled after, not before, GTK dispatch | open | medium | n/a — fork-only (GTK worker dispatch) | — | `dispatch.main_thread` schedules every progress event with `GLib.idle_add`; `_on_progress` applies the 10 Hz UI throttle only after each callback reaches the main loop ([ui/dispatch.py](../ui/dispatch.py), [ui/run_control.py](../ui/run_control.py)). Fast engine iteration can therefore queue a long tail of callbacks that mostly become no-ops but still delay clicks and completion handling. Use a latest-value/coalescing progress dispatcher with forced delivery for phase changes and 100% completion. Verify a large worker-side burst has bounded pending sources and delivers the final value. |
| **F8** | Download completion rebuilds the whole catalogue regardless of visibility | open | medium | n/a — fork-only (Download Center window) | — | `after_batch` calls `refresh_after_downloads` whenever a cached Download Center object exists, including while its window is hidden; that method clears and recreates roughly 500 rows ([ui/download.py](../ui/download.py), [ui/download_center.py](../ui/download_center.py)). When visible, the rebuild resets list scroll position while the user is browsing; when hidden, it creates an avoidable main-loop spike. Refresh installed/available state incrementally or mark the hidden window dirty and rebuild on its next presentation. Preserve filters, checked rows, active tab, and scroll position in the visible case. |
...
*Last reviewed: 2026-08-11 — catalogued GTK refresh-loop and main-loop backlog findings (F5–F8).*
